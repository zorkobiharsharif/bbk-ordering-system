...
// Real push notifications — these reach a device even with the admin app
// fully closed, unlike the broadcast above (which only reaches an already-
// open tab). admin_users!inner + the is_active filter means a disabled
// staff account's devices stop getting pushed to immediately, the same
// moment "Disable" is clicked — no separate cleanup needed.
  ...
  try {
    ...
    await Promise.all((subs || []).map(async (sub: any) => {
      try {
        ...
        // at the OS level, etc.) — remove it instead of failing on every
        // future order forever.
...
Deno.serve(async request => {
  ...
  try {
    ...
    // above only reaches an already-open tab.
